...
resource "aws_s3_bucket" "tf_state_backend_bucket" {
  bucket = "${var.environment}-rsvp-consumer-tfstate-${data.aws_caller_identity.current.account_id}-${var.default_region}"
  acl    = "private"
...
resource "aws_dynamodb_table" "dynamodb-terraform-state-lock" {
  name           = "${var.environment}-rsvp-consumer-tfstate-lock-db-${var.default_region}"
  read_capacity  = 2
  write_capacity = 2
